...
class UserController extends AdminController {
    ...
    public function get_user_list_datatable() {
        $user_list = UserDetails::orderBy('id','DESC')->where('approve_status','1')->get();
    ...
    public function post_edit_user(Request $request, $id) {
        ...
        if ($validator->passes()) {
            ...
            {
              $registration_number = $this->rand_string(8);
              $serial_number = $this->rand_number(8);
              $model->registration_number = $registration_number;
              $model->serial_number = $serial_number;

            $model->save();
            $url = Route('login');
            $email_setting = $this->get_email_data('user_details_approve', array('NAME' => $model->first_name, 'REGISTRATION_NUMBER' => $registration_number, 'SERIAL_NUMBER' => $serial_number , 'URL' => $url));
    ...
    public function get_users_csv() {
        $table = UserDetails::where('approve_status', '=', '1')->get();
