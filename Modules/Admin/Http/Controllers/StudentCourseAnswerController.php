...
class StudentCourseAnswerController extends AdminController {
    ...
    public function datatables() {
        // $datas = Exam::where('course_id','<>','0')->orderBy('id', 'desc')->get();
                                ...
                                if($model->admin_marks_submit == '0'){
                                    ...
                                    if($model->course->exam_status == '0'){
                                        ...
                                        if($model->certificate_delivered == '0'){
                                            return
                                            '<a href="' . Route("admin-student-exam-answer-edit", [$model->id]) . '" class="btn btn-xs btn-primary pull-left"><i class="fa fa-edit"></i> Edit</a>' .
                                            '<a href="' . Route("exam-certificate-download", [$model->id]) . '" class="btn btn-xs btn-primary pull-left"><i class="fa fa-file"></i> Certificate</a>'.
                                            '<a href="' . Route("exam-result-download", [$model->id]) . '" class="btn btn-xs btn-primary pull-left"><i class="fa fa-download"></i> Result</a>'.
                                            '<a href="' . Route("exam-certificate-delivered", [$model->id]) . '" class="btn btn-xs btn-primary pull-left" style="background-color:red;"><i class="fa fa-close"></i> Pending</a>';
                                        }else{
                                            return
                                            '<a href="' . Route("admin-student-exam-answer-edit", [$model->id]) . '" class="btn btn-xs btn-primary pull-left"><i class="fa fa-edit"></i> Edit</a>' .
                                            '<a href="' . Route("exam-certificate-download", [$model->id]) . '" class="btn btn-xs btn-primary pull-left"><i class="fa fa-file"></i> Certificate</a>'.
                                            '<a href="' . Route("exam-result-download", [$model->id]) . '" class="btn btn-xs btn-primary pull-left"><i class="fa fa-download"></i> Result</a>'.
                                            '<a href="' . Route("exam-certificate-delivered", [$model->id]) . '" class="btn btn-xs btn-primary pull-left"><i class="fa fa-check"></i> Delivered</a>';
                                        }
    ...
    public function exam_certificate_delivered($id){
        $model=Exam::where('id',$id)->first();
        return view('admin::exam.certificate_delivered', compact('id','model'));
    }
    ...
    public function post_exam_certificate_delivered(Request $request,$id){
        ...
        if ($validator->passes()) {
            ...
            return redirect()->route('admin-student-exam-answer-index')->with('success_msg', 'Exam Certificate Delivered.');
        } else {
            return redirect()->back()->withErrors($validator)->withInput()->with('error_msg', 'Something went wrong please check your input.');
        }
    }
