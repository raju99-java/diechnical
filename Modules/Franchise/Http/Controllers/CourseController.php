...
class CourseController extends FranchiseController {

    //*** JSON Request
    public function datatables() {
        $id=Auth()->guard('franchise')->user()->id;
        $datas = Course::orderBy('id', 'desc')->where('created_by',$id)->get();
        ...
        return Datatables::of($datas)
                        ...
                        ->addColumn('action', function ($model) {
                            return
                                    '<a href="' . Route("franchise-course-edit", [$model->id]) . '" class="btn btn-xs btn-primary pull-left"><i class="fa fa-edit"></i> Edit</a>' .
                                    '<a href="javascript:;" onclick="deleteCourse(this);" data-href="' . Route("franchise-course-delete", [$model->id]) . '" class="btn btn-xs btn-primary pull-left"><i class="fa fa-trash"></i> Delete</a>'.
                                    '<a href="' . Route("franchise-course-question-answer", [$model->id]) . '" class="btn btn-xs btn-primary pull-left"><i class="fa fa-eye"></i> Question & Answer</a>'.
                                    '<a href="' . Route("franchise-course-module", [$model->id]) . '" class="btn btn-xs btn-primary pull-left"><i class="fa fa-eye"></i> Modules</a>'.
                                    '<a href="' . Route("franchise-course-live-class-list", [$model->id]) . '" class="btn btn-xs btn-primary pull-left"><i class="icofont-video"></i> Live Class</a>';
                        })
                        ->rawColumns(['image','featured','live','status', 'action'])
                        ->toJson(); //--- Returning Json Data To Client Side
    }

    //*** GET Request
    public function index() {
        return view('franchise::course.list');
    }

    //*** GET Request
    public function create() {
        return view('franchise::course.add');
    }
    ...
    public function store(Request $request) {
        ...
        if ($validator->passes()) {
            $data = new Course();
            $input = $request->all();
            $input['created_by'] = Auth()->guard('franchise')->user()->id;
            $input['live'] = '0';
            $input['status'] = '0';
            ...
            return redirect()->route('franchise-course-index')->with('success_msg', 'Course created successfully.');
        } else {
    ...
    public function edit($id) {
        ...
        return view('franchise::course.edit', compact('data'));
    }
    ...
    public function update(Request $request, $id) {
        ...
        if ($validator->passes()) {
            ...
            return redirect()->route('franchise-course-index')->with('success_msg', 'Course updated successfully.');
        } else {
            return redirect()->back()->withErrors($validator)->withInput()->with('error_msg', 'Something went wrong please check your input.');
        }
    ...
    public function question_answer($id) {
        return view('franchise::course.question_answer_list', compact('id'));
    }
    public function module_datatables($id) {
        ...
        return Datatables::of($datas)
                        ...
                        ->addColumn('action', function ($model) {
                            return
                                    '<a href="' . Route("franchise-course-module-edit", [$model->id]) . '" class="btn btn-xs btn-primary pull-left"><i class="fa fa-edit"></i> Edit</a>' .
                                    '<a href="' . Route("franchise-course-module-video", [$model->id]) . '" class="btn btn-xs btn-primary pull-left"><i class="fa fa-eye"></i> Videos</a>' .
                                    '<a href="javascript:;" onclick="deleteCourseModule(this);" data-href="' . Route("franchise-course-module-delete", [$model->id]) . '" class="btn btn-xs btn-primary pull-left"><i class="fa fa-trash"></i> Delete</a>';
                        })
    ...
    public function module($id) {
        ...
        return view('franchise::course.course_module_list', compact('id','course'));
    }
    public function module_add($id) {
        $course= Course::where('id', $id)->first();
        return view('franchise::course.module_add', compact('id','course'));
    }
    public function post_module_add(Request $request,$id) {
        ...
        if ($validator->passes()) {
            ...
            $data1 = Course::findOrFail($id);
            $input1['live'] = '0';
            $data1->update($input1);

            return redirect()->route('franchise-course-module', [$id])->with('success_msg', 'Course module created successfully.');
        } else {
            return redirect()->back()->withErrors($validator)->withInput()->with('error_msg', 'Something went wrong please check your input.');
        }
    }
    ...
    public function module_edit($id) {
        ...
        return view('franchise::course.module_edit', compact('data'));
    }
    ...
    public function post_module_edit(Request $request, $id) {
        ...
        if ($validator->passes()) {
            ...
            $data1 = Course::findOrFail($data->course_id);
            $input1['live'] = '0';
            $data1->update($input1);
            
            return redirect()->route('franchise-course-module', [$data->course_id])->with('success_msg', 'Course module updated successfully.');
        } else {
            return redirect()->back()->withErrors($validator)->withInput()->with('error_msg', 'Something went wrong please check your input.');
        }
        //--- Redirect Section Ends
    ...
    public function module_video_datatables($id) {
        ...
        return Datatables::of($datas)
                        ...
                        ->addColumn('action', function ($model) {
                            return
                                    '<a href="' . Route("franchise-course-module-video-edit", [$model->id]) . '" class="btn btn-xs btn-primary pull-left"><i class="fa fa-edit"></i> Edit</a>' .
                                    '<a href="javascript:;" onclick="deleteCourseModuleVideo(this);" data-href="' . Route("franchise-course-module-video-delete", [$model->id]) . '" class="btn btn-xs btn-primary pull-left"><i class="fa fa-trash"></i> Delete</a>';
                        })
                        ->rawColumns(['status', 'action'])
                        ->make(true); //--- Returning Json Data To Client Side
    }
    ...
    public function module_video($id) {
        ...
        return view('franchise::course.course_module_video_list', compact('id','coursemodule'));
    }
    public function module_video_add($id) {
        $coursemodule= CourseModule::where('id', $id)->first();
        return view('franchise::course.module_video_add', compact('id','coursemodule'));
    }
    public function post_module_video_add(Request $request,$id) {
        ...
        if ($validator->passes()) {
            ...
            $data->fill($input)->save();
            $model = CourseModule::where('id', $id)->first();
            
            $model->updated_at = Carbon::now()->toDateTimeString();
            $model->save();
            
            $data1 = Course::findOrFail($input['course_id']);
            $input1['live'] = '0';
            $data1->update($input1);

            return redirect()->route('franchise-course-module-video', [$id])->with('success_msg', 'Course module video created successfully.');
        } else {
            return redirect()->back()->withErrors($validator)->withInput()->with('error_msg', 'Something went wrong please check your input.');
        }
    }
    ...
    public function module_video_edit($id) {
        ...
        return view('franchise::course.module_video_edit', compact('data'));
    }
    ...
    public function post_module_video_edit(Request $request, $id) {
        ...
        if ($validator->passes()) {
            ...
            return redirect()->route('franchise-course-module-video', [$data->module_id])->with('success_msg', 'Course module video updated successfully.');
        } else {
            return redirect()->back()->withErrors($validator)->withInput()->with('error_msg', 'Something went wrong please check your input.');
        }
        //--- Redirect Section Ends
    ...
    public function live_class_data($id){
        ...
        $datas = LiveClass::where('franchise_id',$fid)->where('course_id',$id)->orderBy('id', 'asc')->get();
        // print_r($datas);
        //--- Integrating This Collection Into Datatables
        return Datatables::of($datas)
                        ->addIndexColumn()
                        ...
                        ->addColumn('action', function ($model) {
                            return
                                    '<a href="javascript:;" onclick="deleteLiveClass(this);" data-href="' . Route("franchise-course-live-class-delete", [$model->id]) . '" class="btn btn-xs btn-primary pull-left"><i class="fa fa-trash"></i> Delete</a>';
                        })
    ...
    public function live_class_list($id){
        $data = Course::findOrFail($id);
        return view('franchise::course.live_class_list', compact('data','id'));
    }
    
    public function get_live_class(Request $request, $id){
        $data = Course::findOrFail($id);
        return view('franchise::course.live_class', compact('data'));
    }
    ...
    public function post_live_class(Request $request, $id){
        ...
        if ($validator->passes()) {
            ...
            return redirect()->route('franchise-course-live-class-list',$id)->with('success_msg', 'Create Successfully.');
        } else {
            return redirect()->back()->withErrors($validator)->withInput()->with('error_msg', 'Something went wrong please check your input.');
        }
    }
